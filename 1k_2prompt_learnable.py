...
# ---------------- Hyper‑parameters -----------------
# ---------------------------------------------------
checkpoint_dir = "2prompt_learnable/16"
...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, content_words, pure_words, n_ctx=3):
        super().__init__()
    ...
    def forward(self, prompts):
        ...
        tok_emb = self.clip_model.token_embedding(tokenized)
        # 计算 EOS 位置
        n_ctx = self.n_ctx
        ctx = self.ctx.unsqueeze(0).expand(tok_emb.size(0), -1, -1)  # [M, n_ctx, D]

        # 插入 ctx：构造新的 token embedding
        # 原始 shape: [M, 77, D]
        prefix = tok_emb[:, :1]             # [M, 1, D] --> 包含起始的 <SOS>
        suffix = tok_emb[:, 1:]             # [M, 76, D] --> 剩余部分
        tok_emb = torch.cat([prefix, ctx, suffix[:, :-self.n_ctx]], dim=1)  # 保持总长度为77
        ...
        content_eos_pos = (content_tok == eos_id).float().argmax(dim=-1) + n_ctx
        pure_eos_pos = (pure_tok == eos_id).float().argmax(dim=-1) + n_ctx
...
def train_one_epoch(model, opt, loader, epoch, scaler):
    ...
    for batch in loop:
        ...
        opt.zero_grad()
        with autocast(device_type='cuda:3'):
